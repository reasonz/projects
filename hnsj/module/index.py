...
def login(username):
    ...
    sql="select id,username,password from HNSJ_TDCMS_USER where username='"+username+"'"
    c = conn.cursor()
    c.execute(sql)
    res = c.fetchone()
    if res :
...
def getmenulist(userid) :
    conn= db.getmysqlconn()
    sql = "SELECT m.id,u.username,u.id,m.menuname,m.PATH,m.PARENT FROM  HNSJ_TDCMS_USER u, HNSJ_TDCMS_MENU m, HNSJ_TDCMS_MENU_USER mu \
