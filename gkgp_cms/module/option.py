...
def getallmenu(): 
    ...
    sql = "select t.id,t.menuname,t.path,t.parent  from gkgp_cms_menu t "
    c = conn.cursor()
    l=[]
    c.execute(sql)
    for o in c.fetchall():
...
def getpower(userid,menuids):
    conn = db.getmysqlconn()
    c=conn.cursor()
    delsql = "delete from gkgp_cms_menu_user where userid="+userid
    c.execute(delsql)
    for menuid in menuids.split(','):
        if len(menuid)>0:
            sql = "insert into gkgp_cms_menu_user (userid,menuid) values(%s,%s)"
...
def adduser():
    ...
    conn=  db.getmysqlconn()
    c=conn.cursor()
    sql="insert into gkgp_cms_user (username,password) values(%s,%s) "
...
def deluser(userids):
    ...
    for one in userids.split(','):
        if len(one) > 0:
            sql="delete from gkgp_cms_user where id=%s"
            c.execute(sql,(one))
            conn.commit()
    c.close()
    conn.close()
...
def adduser():
    ...
    conn=  db.getmysqlconn()
    c=conn.cursor()
    sql="insert into gkgp_cms_menu (menuname,path,parent) values(%s,%s,%s) "
...
def deluser(menuids):
    ...
    for one in menuids.split(','):
        if len(one) > 0:
            sql="delete from gkgp_cms_menu where id="+one
            c.execute(sql)
            conn.commit()
    c.close()
    conn.close()
...
def addproject():
    ...
    conn = db.getmysqlconn()
    c=conn.cursor()
    sql = "insert into gkgp_cms_menu (menuname,parent) values(%s,%s)"
...
def delproject(projectid):
    ...
    for one in projectid.split(','):
        if len(one)>0:
            sql = "delete from gkgp_cms_menu where id = %s"
            c.execute(sql,(one))
            conn.commit()
    c.close()
    conn.close()
